...
public class MoveElevator extends CommandBase
{
    ...
    public boolean isFinished() 
    {
        if(speed>0)
        {
            return getEncLimits() || !RobotContainer.getJoy().getRawButton(Constants.ELEVATOR_BUTTON_UP);
        }
        else
        {
            return (RobotContainer.getElevator().getLimit()) || !RobotContainer.getJoy().getRawButton(Constants.ELEVATOR_BUTTON_DOWN);
        }
